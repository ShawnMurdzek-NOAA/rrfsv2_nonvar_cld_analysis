...
!
  ...
  real, allocatable :: theta(:,:),relhum(:,:),cldfrac(:,:),rho(:,:)
...
!===============================================================================
  ...
  elseif (use_mpi > 0) then
    ...
    if (mype == 0) then
      theta = var
      call MPI_RECV(var,nCell*nVert,MPI_REAL,1,1,MPI_COMM_WORLD,MPI_STATUS_IGNORE,ierr)
      relhum = var
      call MPI_RECV(var,nCell*nVert,MPI_REAL,2,1,MPI_COMM_WORLD,MPI_STATUS_IGNORE,ierr)
      cldfrac = var
      call MPI_RECV(var,nCell*nVert,MPI_REAL,3,1,MPI_COMM_WORLD,MPI_STATUS_IGNORE,ierr)
      rho = var
    elseif (mype == 1) then
      call MPI_SEND(var,nCell*nVert,MPI_REAL,0,1,MPI_COMM_WORLD,ierr)
    elseif (mype == 2) then
      call MPI_SEND(var,nCell*nVert,MPI_REAL,0,1,MPI_COMM_WORLD,ierr)
    elseif (mype == 3) then
      call MPI_SEND(var,nCell*nVert,MPI_REAL,0,1,MPI_COMM_WORLD,ierr)
